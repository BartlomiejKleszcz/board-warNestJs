...
export class UnitsService {
    ...
    resetPlayerUnits(playerId: Player['id']) {
        const remainingUnits = this.customUnits.filter(unit => unit.playerId !== playerId); // pozostale jednostki
    ...
    startGame(config: GameConfig) {
        if (config.mode === 'turns' && (!config.turnLimit || config.turnLimit < 1)) {
            throw new Error('Turn limit must be provided and greater than 0 for turn-based mode.'); // walidacja limitu tur
        }
    ...
    attack(attackerId: number, defenderId: number, mode: 'melee' | 'ranged' = 'melee'): AttackResult {
        ...
        const attackValue = mode === 'ranged' ? attacker.rangedAttack : attacker.meleeAttack; // wybierz statystyke
    ...
    getBattleStats(): BattleStats {
        ...
        const totalDamage = Object.values(totalDamageByPlayer).reduce((acc, curr) => acc + curr, 0); // suma obrazen
