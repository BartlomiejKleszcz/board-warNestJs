...
import { Player as PrismaPlayer } from '@prisma/client'; // typ encji w bazie
...
export class PlayerService {
    ...
    async create(name: string, color: string): Promise<Player> {
        ...
        return this.toDomain(created as PrismaPlayer); // mapowanie na domene
    }

    async findAll(): Promise<Player[]> {
        const players = await this.playerRepository.findAll(); // pobierz wszystkich
        return players.map((p) => {
            const domain = this.toDomain(p as PrismaPlayer); // mapuj
            domain.units = this.unitService.getPlayerUnits(domain.id); // dolacz jednostki z pamięci
    ...
    async findById(id: Player['id']): Promise<Player | undefined> {
        ...
        const domain = this.toDomain(player as PrismaPlayer); // mapuj
    ...
    async updatePlayer(id: Player['id'], name: string, color: string): Promise<Player | undefined> {
        ...
        return updated ? this.toDomain(updated as PrismaPlayer) : undefined; // zwroc zmapowany lub undefined
    }
    ...
    async addUnitToPlayer(playerId: Player['id'], unitIdStr: string): Promise<Player | undefined> {
        ...
        const player = this.toDomain(dbPlayer as PrismaPlayer); // zmapuj domene
    ...
    deleteUnitFromPlayer(playerId: Player['id'], uniqueId: number): Player | undefined {
        ...
        const unitIndex = player.units.findIndex((unit) => unit.uniqueId === uniqueId); // znajdz jednostke
        if (unitIndex !== -1) {
